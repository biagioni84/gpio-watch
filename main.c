...
//#define LONG_PRESS 5000000000LL
...
void run_script (int pin, int value, int interval) {
	char *script_path,
	     pin_str[GPIODIRLEN],
	     value_str[2],
	     interval_str[16];
	...
	LOG_INFO("pin %d: running script %s interval %s", pin, script_path, interval_str);
...
int watch_pins() {
	...
	valbuf[2] = '\0';
	memset(switch_state, 1, num_pins);
	...
	for (i=0; i<num_pins; i++) {
		...
		LOG_INFO("read pins from: %s", pin_path);
		fd = open(pin_path, O_RDONLY);
		read(fd, valbuf, 2);
		fdlist[i].fd = fd;
		fdlist[i].events =  POLLIN|POLLPRI;
	}
	...
	while (1) {
		...
		for (i=0; i<num_pins; i++) {
			if (fdlist[i].revents & (POLLIN|POLLPRI)) {
				//LOG_INFO("pin %d: received event", pins[i].pin);
				lseek(fdlist[i].fd, 0, SEEK_SET);
				read(fdlist[i].fd, valbuf, 2);
				//LOG_INFO("value: %c", valbuf[0]);
				// continue;
				...
				// DEBOUNCE_INTERVAL nanoseconds after the '0' event.
					...
					if (switch_state[i] == 1 && valbuf[0] == '0') {
						down_at[i] = now;
						switch_state[i] = 0;
					} else if (switch_state[i] == 0 && valbuf[0] == '1') {
